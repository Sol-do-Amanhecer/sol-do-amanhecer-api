...
class EmailUtilTest {
    ...
    class EnvioComSucesso {

        @Test
        @DisplayName("Deve registrar mensagens de sucesso")
        public void deveRegistrarMensagensDeSuccesso() {
            when(javaMailSender.createMimeMessage()).thenReturn(mimeMessage);
            doNothing().when(javaMailSender).send(any(MimeMessage.class));

            assertDoesNotThrow(() -> emailUtil.enviarEmail(DESTINATARIO, ASSUNTO, MENSAGEM));
        ...
        @Test
        @DisplayName("Deve enviar e-mail com caracteres especiais")
        public void deveEnviarEmailComCaracteresEspeciais() {
            when(javaMailSender.createMimeMessage()).thenReturn(mimeMessage);
            doNothing().when(javaMailSender).send(any(MimeMessage.class));

            String destinatarioComCaracteresEspeciais = "usuario+teste@dominio.com.br";
            String assuntoComCaracteresEspeciais = "Assunto com acentos: cao, a, e";
            String mensagemComHTML = "<p>Mensagem com <strong>HTML</strong></p>";

            assertDoesNotThrow(() -> emailUtil.enviarEmail(
                    destinatarioComCaracteresEspeciais, assuntoComCaracteresEspeciais, mensagemComHTML));
    ...
    @Nested
    @DisplayName("Falhas no envio")
    class FalhasNoEnvio {

        @Test
        @DisplayName("Deve lancar UsuarioException quando createMimeMessage falha")
        public void deveLancarExcecaoQuandoCreateMimeMessageFalha() {
        ...
        @Test
        @DisplayName("Deve lancar UsuarioException quando send falha")
        public void deveLancarExcecaoQuandoSendFalha() {
            when(javaMailSender.createMimeMessage()).thenReturn(mimeMessage);
        ...
        @Test
        @DisplayName("Deve lancar UsuarioException quando MessagingException ocorre")
        public void deveLancarExcecaoQuandoMessagingExceptionOcorre() {
            when(javaMailSender.createMimeMessage()).thenReturn(mimeMessage);
            doThrow(new RuntimeException("Erro de configuracao")).when(javaMailSender).send(any(MimeMessage.class));
    ...
    @Nested
    @DisplayName("Entrada invalida")
    class EntradaInvalida {

        @Test
        @DisplayName("Deve tratar parametros nulos sem falhar")
        public void deveTratarParametrosNulos() {
            RuntimeException causaOriginal = new RuntimeException("Parametro invalido");
            when(javaMailSender.createMimeMessage()).thenThrow(causaOriginal);
