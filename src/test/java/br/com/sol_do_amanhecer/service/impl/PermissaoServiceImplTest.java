...
class PermissaoServiceImplTest {
    ...
    class BuscarPorId {
        ...
        void buscarPorId_deveRetornarPermissaoDTO_quandoEncontrar() {
            ...
            assertAll("Resultado deve conter os dados da permissão buscada",
                    () -> assertThat(resultado).as("O resultado não deve ser nulo").isNotNull(),
                    () -> assertThat(resultado.getUuid()).as("O UUID deve corresponder ao ID buscado").isEqualTo(id),
                    () -> assertThat(resultado.getDescricao()).as("A descrição deve ser ROLE_USER").isEqualTo("ROLE_USER")
            );
